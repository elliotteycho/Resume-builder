...
- Generalize company-internal codenames and confidential partner/client names into plain descriptors a stranger understands: an internal tool codename becomes "internal LLM agent platform" or "internal ATS", a confidential client becomes "the lowest fill-rate hotel partner". Keep publicly recognizable technologies verbatim (Amazon Redshift, Mode, Figma, Django, Python). The experience bank may carry internal names as context — they never appear verbatim on the resume. If the bank's profile carries a glossary mapping internal names to descriptors, use it.
...
- Fill the line: aim for 170-200 characters per bullet (two full lines at 11pt) — a bullet that ends mid-line wastes keyword space. Use the room for role-relevant keywords and additional verified metrics, never filler. Under ~150 reads thin; never exceed 210.
- Pack metric density: when an entry's evidence supports multiple numbers, carry 2-3 verified metrics per bullet rather than one — every number must still be copied verbatim from evidence.
- Every bullet leads with a STRONG action verb from the assigned register — never weak openers like "Helped", "Assisted", "Worked on", "Participated in", "Responsible for".
